...
var JsonRpcClient = (function() {
	...
	p.setAuth = function(username, password) {
	...
	p.schedule = function(success, unsuccess) {
		...
		function refresh() {
			client.jqXHR = client.createAjaxObj().done(
					function(data, status, jqxhr) {
						client.request.done(data, status, jqxhr);
						client.isSuccess = true;
					}).fail(
					function(data, textStatus, errorThrown) {
						printError(data, textStatus, errorThrown,
								client.request.url, client.request.method,
	...
	p.createAjaxObj = function() {
		...
		if (client.username) {
			tmp.headers = {
				"Authorization" : "Basic "
						+ btoa(client.username + ":" + client.password)
			}
		}
		var result = $.ajax(tmp);
		return result;
